...
import { AlertDialog, Card, Flex, oneToMojo, mojoToOneLocaleString } from '@one/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const oneper_input = ip_parsed.limit;
    ...
    const oneper_value = Number.parseInt(Number(oneper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (one per interval):{' '}
                  {mojoToOneLocaleString(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToOneLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = oneToMojo(amount_input.value);
    const fee = oneToMojo(fee_input.value);
