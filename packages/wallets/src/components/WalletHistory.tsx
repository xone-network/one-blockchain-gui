import { WalletType, TransactionType, toBech32m } from '@xone-network/api';
import { useGetOfferRecordMutation, useGetSyncStatusQuery } from '@xone-network/api-react';
import {
  Card,
  CopyToClipboard,
  Flex,
...
import type { Row } from '@xone-network/core';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
    ...
    field: (row: Row, metadata) => {
      ...
      return (
        <>
          ...
          &nbsp;
          <strong>
            <FormatLargeNumber value={type === WalletType.CAT ? mojoToCAT(row.amount) : mojoToOne(row.amount)} />
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>
          <FormatLargeNumber value={mojoToOne(row.feeAmount)} />
        </strong>
        &nbsp;
        {metadata.feeUnit}
      </>
