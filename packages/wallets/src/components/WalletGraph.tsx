import { TransactionType, WalletType } from '@xone-network/api';
import type { Transaction } from '@xone-network/api';
import { useGetWalletBalanceQuery } from '@xone-network/api-react';
import { mojoToOne, mojoToCAT, blockHeightToTimestamp } from '@xone-network/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToOne(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: (walletType === WalletType.CAT ? mojoToCAT(balance) : mojoToOne(balance)).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToOne(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: walletType === WalletType.CAT ? mojoToCAT(start) : mojoToOne(start).toString(), // bignumber is not supported by react
    });
  });
