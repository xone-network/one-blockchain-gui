...
import { TransactionType, WalletType } from '@one/api';
import type { Transaction } from '@one/api';
import { mojoToOne, mojoToCAT, blockHeightToTimestamp } from '@one/core';
  ...
  data.forEach((item) => {
    ...
    points.push({
      ...
      tooltip:
        walletType === WalletType.CAT
          ? mojoToCAT(start)
          : mojoToOne(start).toString(), // bignumber is not supported by react
    });
  });
