...
import { AlertDialog, oneToMojo } from '@one/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      oneper_input.value === '' ||
      Number(oneper_input.value) === 0 ||
      !Number(oneper_input.value) ||
      isNaN(Number(oneper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const oneper = oneToMojo(oneper_input.value);
    const oneper_value = Number.parseInt(Number(oneper));
    ...
    // var fee = oneToMojo(fee_input.value);
