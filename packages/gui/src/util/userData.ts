...
export function getUserDataDir(): string {
  const oneRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(oneRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const oneRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${oneRootUserDataPath}`);
  app.setPath('userData', oneRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(oneRootUserDataPath, 'Local Storage', 'leveldb');
