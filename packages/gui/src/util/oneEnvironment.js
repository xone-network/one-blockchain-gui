...
const getOneVersion = () => {
  let version = null;
  const exePath = getExecutablePath('one');
  // first see if we can get a one exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or one is on the path
    try {
      version = childProcess
