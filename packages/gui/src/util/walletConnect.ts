...
const log = initDebug('one-gui:walletConnect');

const availableCommands = walletConnectCommands.map((command) => `one_${command.command}`);
...
export async function processSessionProposal(
  ...
  try {
    ...
    const requiredNamespace = requiredNamespaces.one;
    if (!requiredNamespace) {
      throw new Error('Missing required one namespace');
    }

    const { chains, methods } = requiredNamespace;
    const chain = chains.find((item) => ['one:testnet', 'one:mainnet'].includes(item));
    if (!chain) {
    ...
    const accounts = fingerprints.map((fingerprint) => `one:${instance}:${fingerprint}`);
