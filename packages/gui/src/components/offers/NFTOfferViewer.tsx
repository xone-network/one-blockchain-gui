...
import { OfferSummaryRecord, OfferTradeRecord } from '@xone-network/api';
import { useCheckOfferValidityMutation, useGetNFTInfoQuery, useGetNFTWallets } from '@xone-network/api-react';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] = useMemo(
    () =>
      Object.keys(summaryData).map((key) => {
        ...
        if (['xone', 'txone'].includes(key.toLowerCase())) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.ONE: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const xoneMakerFee = mojoToOne(makerFee);

    return {
      ...calculateNFTRoyalties(
        amount,
        parseFloat(xoneMakerFee),
  ...
  const overrideNFTSellerAmount =
    exchangeType === NFTOfferExchangeType.TokenForNFT
      ? assetType === OfferAsset.ONE
        ? oneToMojo(nftSaleInfo?.nftSellerNetAmount ?? 0)
