import { type OfferSummaryRecord } from '@xone-network/api';
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToOne, mojoToCAT } from '@xone-network/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['xone', 'txone'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToOne(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['xone', 'txone'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToOne(takerEntries[0][1])
