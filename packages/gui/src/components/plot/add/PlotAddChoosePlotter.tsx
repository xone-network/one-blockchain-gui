import { defaultPlotter, PlotterName } from '@xone-network/api';
import type { Plotter, PlotterMap } from '@xone-network/api';
import { useGetPlottersQuery } from '@xone-network/api-react';
import { CardStep, Select, StateColor } from '@xone-network/core';
...
export default function PlotAddChoosePlotter(props: Props) {
  ...
  function getDisplayablePlotters(p: PlotterMap<PlotterName, Plotter>): PlotterName[] {
    ...
    displayablePlotters.sort((a, b) => (a === PlotterName.ONEPOS ? -1 : a.localeCompare(b)));
    ...
    <CardStep step={step} title={<Trans>Choose Plotter</Trans>}>
      <Typography variant="subtitle1">
        <Trans>
          ...
          the default One Proof of Space plotter. If unsure, use the default One Proof of Space plotter.
        </Trans>
      </Typography>
