import { useGetFeeEstimateQuery } from '@xone-network/api-react';
...
import mojoToOneLocaleString from '../../utils/mojoToOneLocaleString';
import Fee from '../Fee';
...
export default function EstimatedFee(props: FeeProps) {
  ...
  const maxBlockCostCLVM = 11_000_000;

  const txCostEstimates = {
    walletSendXONE: Math.floor(maxBlockCostCLVM / 1170),
  ...
  const formatEst = useCallback(
    (number: number, multiplierLocal: number, localeLocal: string) => {
      ...
      return mojoToOneLocaleString(num, localeLocal);
    },
    [multiplyEstimate]
  );
  ...
  const formattedEstimates: FormattedEstimate[] = useMemo(() => {
    ...
    const defaultValues = [6_000, 5_000, 0];
  ...
  useEffect(() => {
    if (formattedEstimates) {
      if (selectedTime) {
        ...
        if (estimate) {
          const xoneFee = mojoToOneLocaleString(estimate.estimate, 'en-US');
          setSelectedValue(estimate.formattedEstimate);
          setValue(name, xoneFee);
        }
      }
    }
